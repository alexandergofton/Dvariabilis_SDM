---
title: "Step 5c: Sensitivity Models — Alternative Variable Sets"
...
This script refits MaxEnt and BRT models using alternative variable sets
defined in Step 4b, then projects to Australia and compares with the
original models. The goal is to test whether removing Bio1 (and Bio7)
improves model agreement and produces more ecologically plausible
...
## 2. Function to fit and evaluate a complete model set
...
brt_C <- fit_brt(occ_env, bg_env, set_C, "Set C")
eval_C <- evaluate_models(maxent_C$model, brt_C, occ_env, bg_env, set_C)
```
...
sprintf("Projections complete")
```

## 6. MESS for alternative variable sets
...
sprintf("MESS novelty — Set B: %.1f%% | Set C: %.1f%%", pct_novel_B, pct_novel_C)
```

## 7. Calculate suitable areas
...
sprintf("Australian prediction correlation — Set B: %.3f | Set C: %.3f", B_aus_cor, C_aus_cor)
```

## 8. Comparison maps

```{r}
#| label: comparison-maps
#| fig-width: 14
#| fig-height: 16
