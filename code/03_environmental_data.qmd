---
title: "Step 3: Environmental Data Preparation"
...
This script loads and prepares environmental (climate) data for the *D. variabilis*
SDM from three sources:

1. **WorldClim 2.1** bioclimatic variables (Bio1-19) at 2.5 arcmin
2. **ENVIREM** derived variables (aridity, PET, continentality)
3. **CHELSA** vapor pressure deficit (VPD) monthly data
...
chelsa_dir <- file.path(climate_dir, "chelsa")
```
...
sprintf("Occurrences: %d | Background: %d", nrow(occ), nrow(bg))
```

## 3. Load WorldClim bioclimatic variables

We load Bio1-19, but **exclude Bio8, Bio9, Bio18, Bio19** which have known spatial
artifacts in WorldClim (temperature/precipitation quarter assignment discontinuities).
...
## 5. Load CHELSA vapor pressure deficit — compute annual mean
...
sprintf("Computed annual mean VPD from %d monthly layers", nlyr(vpd_monthly))
```

## 6. Harmonise raster extents and resolutions

All layers need to match the WorldClim grid (2.5 arcmin, global extent).
ENVIREM is already at 2.5 arcmin. CHELSA may differ slightly and needs resampling.
...
## 7. Crop to training extent (accessible area M)
...
## 8. Crop to projection extent (Australia)

```{r}
#| label: crop-australia
...
## 9. Extract environmental values at occurrence and background points
...
sprintf("After removing NAs — Occurrences: %d | Background: %d",
        nrow(occ_env_clean), nrow(bg_env_clean))
```

## 10. Summary of environmental values at occurrence points
...
## 11. Visualisation — environmental variable distributions

Compare presence vs background distributions for key variables.

```{r}
#| label: env-distributions
#| fig-width: 12
#| fig-height: 10
...
sprintf("Total candidate variables: %d", nlyr(env_all))
```
