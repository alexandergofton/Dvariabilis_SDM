---
title: "Step 10: Seasonal Activity Projection for Australia"
...
The annual MaxEnt and BRT models (Steps 5–7) identify *where* in Australia
*D. variabilis* could establish climatically. This script answers *when* — predicting
which months of the year adult questing activity would be expected in climatically
suitable Australian regions, and mapping the peak activity window.

**Approach: Climate-matching seasonal activity model**

We cannot project the annual MaxEnt/BRT models per-month (their predictors are
annual aggregates). Instead, we:

1. Derive an empirical **activity ~ monthly mean temperature** response from native-range
   occurrence points weighted by literature-documented monthly activity intensity
2. Derive a secondary **precipitation weight** (drought suppression filter) from the
   same data
3. Project both to Australian monthly WorldClim rasters to produce 12 monthly activity layers
4. Mask to the **union of MaxEnt and BRT** annual suitability zones, with MESS novel-climate
   masking applied
...
Note: All peaks correspond to late boreal spring/early summer. In the southern hemisphere,
the climatologically equivalent window is October–December, but the exact Australian
peak months are determined by the climate-matching, not a fixed calendar offset.
...
## 3. Extract monthly climate at occurrence points

For each occurrence point, extract monthly mean temperature (tmean = (tmin + tmax) / 2)
and monthly precipitation for all 12 months.

```{r}
#| label: extract-monthly-occ
...
sprintf("Extracted monthly climate at %d occurrence points x 12 months", nrow(occ_tmean))
```

## 4. Build weighted activity dataset

Create one row per (occurrence × month) observation. Each row gets the
literature activity weight for that month. Only months with weight > 0 are
retained for GAM fitting, but zero-weight anchor points are added at
temperature extremes to ensure the curve returns to zero.

```{r}
#| label: build-activity-data
...
## 5. Fit activity response curve: activity ~ tmean (GAM)

We fit a GAM through the weighted (occurrence × month) data. Each point has
`y = activity_weight[month]` and `weight = activity_weight`. This produces a smooth
empirical curve of expected activity as a function of monthly mean temperature.

Zero-activity anchor points are added at low (~-5°C) and high (>32°C) temperatures
to prevent the GAM from predicting high activity in temperature ranges that are
never associated with activity in the native range.
...
sprintf("Fitted curve peak temperature: %.0f°C (expected: 15–22°C)", peak_temp)
```

## 6. Plot activity response curve
...
From occurrence points during active months, the 5th percentile of monthly
precipitation defines the drought suppression threshold — below this, activity
is down-weighted linearly to zero.

```{r}
#| label: prec-threshold
...
## 8. Load and crop Australian monthly climate

```{r}
#| label: load-aus-monthly
...
## 9. Project monthly activity to Australia

For each month, apply:
1. Temperature activity score from GAM
2. Precipitation weight (drought suppression)
3. Combined activity = temperature score × precipitation weight
...
## 10. Build the annual suitability mask
...
sprintf("Final mask (suitable & non-novel): %d (%.1f%%)", n_final, 100 * n_final / n_total)
```

## 11. Apply mask to monthly activity rasters
...
sprintf("Masked monthly activity raster: %d layers", nlyr(monthly_masked))
```

## 12. Derive summary rasters

```{r}
#| label: summary-rasters
...
## 13. 12-panel monthly activity map
...
Extract the monthly activity profile for key Australian regions and compare
against the known native-range phenology pattern.

```{r}
#| label: regional-phenology
...
Apply the fitted activity function to monthly WorldClim rasters for the **native range**
to verify that the model correctly identifies May–June as peak months at known
study sites. This is a key sanity check.
...
```{r}
#| label: plot-back-validate
#| fig-width: 12
#| fig-height: 5

p_validate <- ggplot(site_long, aes(x = month, y = activity, group = site, colour = site)) +
...
sprintf("Saved seasonal activity outputs to processed_data/ and figures/")
```
